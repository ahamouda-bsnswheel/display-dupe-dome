...
const Profile = ({ readOnly: propReadOnly }: ProfileProps = {}) => {
  ...
  const fetchOrgChartData = async () => {
    ...
    try {
      const headers = authStorage.getAuthHeaders();
      const response = await fetch(`https://bsnswheel.org/api/v1/org_chart/custom/${employeeId}`, {
        method: "PUT",
        headers,
      });
  ...
  const fetchEmployeeDetails = async () => {
    ...
    try {
      const headers = authStorage.getAuthHeaders();
      const response = await fetch(`https://bsnswheel.org/api/v1/employee_details/custom/${employeeId}`, {
        method: "PUT",
        headers,
      });
      ...
      <ScrollArea className={`flex-1 ${(isReadOnly && routeEmployeeId && employeeData?.approval_state === "submitted") || (!isReadOnly && hasPendingChanges) ? "pb-20" : ""}`}>
        ...
        <div className="relative bg-gradient-hero px-4 sm:px-6 md:px-8 py-6 sm:py-8 md:py-10 overflow-hidden">
          ...
          <div className="relative flex flex-col items-center mb-4">
            ...
            <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-1 drop-shadow-lg text-center px-4">
              {user.name}
            </h2>
            <p
              className={`text-xs sm:text-sm text-white/90 mb-1 flex items-center gap-1.5 drop-shadow ${isRTL ? "flex-row-reverse" : ""}`}
            >
              <Phone className="h-3 w-3 sm:h-4 sm:w-4" />
              <span className="truncate max-w-[200px] sm:max-w-none">{user.employeeId}</span>
            </p>
            <p
              className={`text-xs sm:text-sm text-white/90 flex items-center gap-1.5 drop-shadow ${isRTL ? "flex-row-reverse" : ""}`}
            >
        ...
        <Tabs value={activeTab} onValueChange={(value) => setSearchParams({ tab: value })} className="w-full">
          ...
          <TabsContent value="work-info" className="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6 mt-0">
            ...
            {/* Approvers */}
            <div>
              <h3 className={`text-base font-semibold text-secondary mb-3 ${isRTL ? "text-right" : ""}`}>
                {t("profile.approvers")}
              </h3>
            ...
            <div>
              ...
              <div className="bg-card p-4 rounded-lg border border-primary/20">
                <div className={`flex items-center gap-3 mb-3 ${isRTL ? "flex-row-reverse" : ""}`}>
                  ...
                  <p className={`text-sm font-semibold text-primary ${isRTL ? "text-right" : ""}`}>
                    {t("profile.roles")}
                  </p>
                </div>
          ...
          <TabsContent value="achievements" className="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6">
            {/* User Level Section */}
            ...
            {/* Badges Section */}
            <div>
              <h3 className={`text-lg font-semibold text-foreground mb-4 ${isRTL ? "text-right" : ""}`}>
                {t("profile.badges")}
              </h3>
              <div className="space-y-3">
                {badges.length === 0 ? (
                  <p className={`text-center text-muted-foreground py-8 ${isRTL ? "text-right" : ""}`}>
                    {t("profile.noBadges")}
                  </p>
                ) : (
                  badges.map((badge) => (
